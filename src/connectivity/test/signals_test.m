function x=signals_test(db_noise)

%load erp_signal.mat; x_erp=x';  
x=zeros(2000,15);
...
%xs(1:1000,1)=x_erp(:,1); xs(1001:1100,1)=0;
...
x(samples_delay+1:1100,3)=x(1:1100-samples_delay,2); x(:,3)=awgn(x(:,3),db_noise,'measured','db');
x(samples_delay+1:1100,4)=x(1:1100-samples_delay,3); x(:,4)=awgn(x(:,4),db_noise,'measured','db');
