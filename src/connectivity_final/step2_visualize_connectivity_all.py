...
def plot_multiband_grid(dtf_bands, pdc_bands, subject_name, epoch_idx,
    ...
    for row, (bands_dict, row_label) in enumerate([
        ...
        for col, band in enumerate(BANDS):
            ...
            if col == 0:
                ax.set_yticklabels(chan, fontsize=5)
            else:
                ax.set_yticklabels([])
            if row == 1:
                ax.set_xticklabels(chan, fontsize=5, rotation=90)
            else:
                ax.set_xticklabels([])
            if col == 0:
...
def plot_subject_timeline(dtf_integrated, pdc_integrated, labels, time_from_onset,
                          subject_name, out_path):
    n_epochs, K, _ = dtf_integrated.shape
    mask = ~np.eye(K, dtype=bool)
    
    mean_dtf = np.array([dtf_integrated[e][mask].mean() for e in range(n_epochs)])
    mean_pdc = np.array([pdc_integrated[e][mask].mean() for e in range(n_epochs)])
    ...
    for lbl, lname, lc in [(0, 'Pre-ictal', LABEL_COLORS[0]),
        ...
        idx = np.where(labels == lbl)[0]
        if len(idx) > 0:
            ax.scatter(t[idx], mean_dtf[idx],
                       c=lc, s=15, marker='o', label=f'DTF {lname}',
                       alpha=0.7, zorder=3)
            ax.scatter(t[idx], mean_pdc[idx],
                       c=lc, s=15, marker='^', label=f'PDC {lname}',
                       alpha=0.4, zorder=3)

    ax.plot(t, mean_dtf, color='steelblue', lw=0.6, alpha=0.4, zorder=2)
    ax.plot(t, mean_pdc, color='tomato',    lw=0.6, alpha=0.4, 
            linestyle='--', zorder=2)
