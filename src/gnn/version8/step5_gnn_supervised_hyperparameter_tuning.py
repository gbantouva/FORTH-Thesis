...
5. HYPERPARAMETER TUNING (GCN with Features only):
   ──────────────────────────────────────────────────────────────────────
   WHY only the GCN, not the baselines:
   The baseline ML models (RF, SVM, SVM-RFE) use hyperparameters fixed
   from published EEG/BCI literature (e.g. C=1.0, max_depth=6).
   With only 8 LOPO folds, any grid search on the baselines would
   inflate variance enormously and provide no reliable signal. Those
   choices are already justified a priori (see step4 docstring).

   For the GCN, tuning IS justified because:
   - The model has more capacity-controlling parameters (hidden, dropout, lr)
   - Architectural choices interact non-linearly (hidden × dropout × lr)
   - The inner validation patient provides a leakage-free signal
   - We use the MEAN inner_val_auc (across all 8 outer folds) as the
     selection criterion — the outer test AUC is NEVER used for selection.

   TUNING PROTOCOL — CRITICAL FOR THESIS INTEGRITY:
   ┌────────────────────────────────────────────────────────────────────┐
   │  For each param combination:                                       │
   │    Run full 8-fold LOPO                                            │
   │    Score = mean(inner_val_auc)  ← selection criterion             │
   │  Select best combination                                           │
   │  Run ONE final evaluation with best params → outer test AUC       │
   │                                                                    │
   │  NEVER select based on outer test AUC. This would be data leakage.│
   └────────────────────────────────────────────────────────────────────┘

   PARAM GRID (kept small — 8 folds × N combos is expensive):
     hidden:         [16, 32, 64]
     dropout:        [0.3, 0.4, 0.5]
     lr:             [0.001, 0.0005]
     threshold_pct:  [60, 70, 80]   (DTF sparsity)

   Default: run the SINGLE best config only (--tune flag enables grid).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  hyperparameter_tuning_results.png   (only when --tune is used)
      Heatmaps of mean inner_val_auc per param combination.

Outputs:
  results_gcn_with_features.json
  results_gcn_adj_only.json
  results_gcn.json              (alias for step6 compatibility)
  hyperparameter_tuning_results.json  (only when --tune is used)
...
Usage — with hyperparameter tuning (GCN with features only):
  python step5_gnn_supervised.py \\
      --featfile      features/features_all.npz \\
      --outputdir     results/gnn_supervised \\
      --tune \\
  ...
  The --tune_epochs flag uses fewer epochs during the tuning grid search
  (recommended: 100) to keep runtime manageable, then the final model
  trains with the full --epochs (150) count.
"""
...
def plot_tuning_results(tuning_records: list, output_dir: Path):
    """
    Visualise hyperparameter tuning results.

    Produces:
      1. Bar chart: mean inner_val_auc per configuration (ranked).
      2. Heatmap: hidden × dropout interaction (mean across lr & threshold_pct).
      3. Heatmap: lr × threshold_pct interaction (mean across hidden & dropout).

    Selection criterion is ALWAYS mean inner_val_auc — never outer test AUC.
    This function is called BEFORE the final evaluation on the outer test set,
    so it can only show inner_val_auc scores, not test AUC.
    """
    if not tuning_records:
        return

    records = sorted(tuning_records, key=lambda r: r['mean_inner_val_auc'], reverse=True)

    # ── 1. Bar chart of all configurations ────────────────────
    labels = [f"h={r['params']['hidden']} d={r['params']['dropout']} "
              f"lr={r['params']['lr']} thr={r['params']['threshold_pct']}"
              for r in records]
    scores = [r['mean_inner_val_auc'] for r in records]
    stds   = [r['std_inner_val_auc']  for r in records]
    colors = ['gold' if i == 0 else 'steelblue' for i in range(len(records))]

    fig, ax = plt.subplots(figsize=(max(10, len(records) * 0.9), 5))
    bars = ax.bar(range(len(records)), scores, yerr=stds,
                  color=colors, edgecolor='black', alpha=0.85, capsize=4)
    ax.set_xticks(range(len(records)))
    ax.set_xticklabels(labels, rotation=45, ha='right', fontsize=8)
    ax.set_ylabel('Mean inner_val_auc (across 8 folds)', fontsize=11)
    ax.set_ylim(max(0, min(scores) - 0.05), min(1.05, max(scores) + 0.08))
    ax.axhline(scores[0], color='gold', linestyle='--', lw=1, alpha=0.6)
    ax.set_title('Hyperparameter tuning — GCN with Features\n'
                 'Selection criterion: mean inner_val_auc  '
                 '(outer test AUC NOT used for selection)',
                 fontsize=11, fontweight='bold')
    ax.text(0, scores[0] + stds[0] + 0.005,
            f'Best: {labels[0]}\nAUC={scores[0]:.4f}±{stds[0]:.4f}',
            fontsize=8, color='darkgoldenrod',
            bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))
    ax.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    plt.savefig(output_dir / 'tuning_bar_chart.png', dpi=150, bbox_inches='tight')
    plt.close()

    # ── 2. Heatmap: hidden × dropout ──────────────────────────
    hiddens  = sorted(set(r['params']['hidden']  for r in records))
    dropouts = sorted(set(r['params']['dropout'] for r in records))
    matrix   = np.full((len(dropouts), len(hiddens)), np.nan)
    for r in records:
        i = dropouts.index(r['params']['dropout'])
        j = hiddens.index(r['params']['hidden'])
        # Take mean across all lr and threshold_pct values for this combo
        same = [x['mean_inner_val_auc'] for x in records
                if x['params']['hidden'] == r['params']['hidden']
                and x['params']['dropout'] == r['params']['dropout']]
        matrix[i, j] = float(np.mean(same))

    fig, ax = plt.subplots(figsize=(6, 4))
    sns.heatmap(matrix, annot=True, fmt='.3f', cmap='YlOrRd',
                xticklabels=hiddens, yticklabels=dropouts, ax=ax,
    ...
    ax.set_title('Mean inner_val_auc\nhidden × dropout (averaged over lr & threshold_pct)',
                 fontsize=10, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_dir / 'tuning_heatmap_hidden_dropout.png', dpi=150, bbox_inches='tight')
    plt.close()

    # ── 3. Heatmap: lr × threshold_pct ────────────────────────
    lrs    = sorted(set(r['params']['lr']            for r in records))
    thrs   = sorted(set(r['params']['threshold_pct'] for r in records))
    matrix2 = np.full((len(thrs), len(lrs)), np.nan)
    for r in records:
        i = thrs.index(r['params']['threshold_pct'])
        j = lrs.index(r['params']['lr'])
        same = [x['mean_inner_val_auc'] for x in records
                if x['params']['lr'] == r['params']['lr']
                and x['params']['threshold_pct'] == r['params']['threshold_pct']]
        matrix2[i, j] = float(np.mean(same))

    fig, ax = plt.subplots(figsize=(5, 4))
    sns.heatmap(matrix2, annot=True, fmt='.3f', cmap='YlOrRd',
                xticklabels=[str(lr) for lr in lrs],
                yticklabels=[str(t) for t in thrs], ax=ax,
                vmin=matrix2[~np.isnan(matrix2)].min() - 0.01,
                vmax=matrix2[~np.isnan(matrix2)].max() + 0.01)
    ax.set_xlabel('learning rate', fontsize=11)
    ax.set_ylabel('threshold_pct', fontsize=11)
    ax.set_title('Mean inner_val_auc\nlr × threshold_pct (averaged over hidden & dropout)',
                 fontsize=10, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_dir / 'tuning_heatmap_lr_threshold.png', dpi=150, bbox_inches='tight')
    plt.close()

    print(f'\n  Tuning plots saved:')
    print(f'    tuning_bar_chart.png')
    print(f'    tuning_heatmap_hidden_dropout.png')
    print(f'    tuning_heatmap_lr_threshold.png')
...
def run_gcn_lopo(model_tag, in_dim, node_feats_or_none, adj_dtf_all,
                 ...
                 save_plots=True,
                 hidden=None, dropout=None, lr=None, threshold_pct=None,
                 epochs=None):
    """
    ...
    Parameters hidden, dropout, lr, threshold_pct, epochs override args
    when provided (used by the tuning loop). When None, args values are used.

    save_plots=False suppresses per-fold plots during the tuning grid search
    to avoid creating thousands of image files.
    """
    # ── Resolve hyperparams: explicit override > args ──────────
    _hidden        = hidden        if hidden        is not None else args.hidden
    _dropout       = dropout       if dropout       is not None else args.dropout
    _lr            = lr            if lr            is not None else args.lr
    _threshold_pct = threshold_pct if threshold_pct is not None else args.threshold_pct
    ...
    if save_plots:
        ...
        print(f'  hidden={_hidden}  dropout={_dropout}  lr={_lr}')
        print(f'  Threshold: p{_threshold_pct:.0f} percentile of inner-train DTF (data-driven)')
        print(f'  Inner val: rotates across folds (no fixed validator)')
        print(f'  Overfitting controls: dropout={_dropout}, weight_decay=1e-4,')
        print(f'  gradient_clipping=1.0, early_stopping(patience={_patience}),')
        print(f'  ReduceLROnPlateau, hidden={_hidden} (~{_hidden*_hidden*2//1000 + 1}k params)')
        print(f'{"=" * 65}')
    ...
    for fold_idx, pat in enumerate(sorted_all_pats):
        ...
        threshold = compute_threshold(adj_dtf_all[it_idx], _threshold_pct)

        if save_plots:
            print(f'\n  Outer fold {fold_idx + 1}/{len(sorted_all_pats)} '
                  f'— Test: {pat}  |  Inner val: {inner_val_pat}  '
                  f'|  Threshold: {threshold:.4f} (p{_threshold_pct:.0f})')
        ...
        model     = SmallGCN(in_dim=in_dim, hidden=_hidden,
                             dropout=_dropout).to(device)
        optimiser = Adam(model.parameters(), lr=_lr, weight_decay=1e-4)
...
# ─────────────────────────────────────────────────────────────
# Hyperparameter tuning — GCN with Features only
# ─────────────────────────────────────────────────────────────

# ── Default param grid ─────────────────────────────────────────────────────
# Kept deliberately small: 3×3×2×3 = 54 combinations × 8 folds = 432 runs.
# For a thesis with 8 patients this is already substantial. Reduce the grid
# if runtime is a concern (e.g. keep only hidden and dropout).
#
# CRITICAL: These values are tried ONLY on the inner_val_auc signal.
# The outer test AUC is computed ONCE at the end using the selected params.
# ──────────────────────────────────────────────────────────────────────────
TUNE_PARAM_GRID = {
    'hidden':         [16, 32, 64],
    'dropout':        [0.3, 0.4, 0.5],
...
def run_hyperparameter_tuning(node_feats, adj_dtf_all, y, patient_ids,
                               args, device, output_dir):
    """
    Grid search over TUNE_PARAM_GRID for the GCN with Features model.

    Selection criterion: mean inner_val_auc across all 8 outer folds.
    The outer test AUC is NEVER used during selection — only after the
    best params are identified and a single final LOPO run is done.

    Parameters
    ----------
    All data/args as passed from main().

    Returns
    -------
    best_params : dict   — best hyperparameter combination
    tuning_records : list — all combinations with their mean inner_val_auc
    """
    keys   = list(TUNE_PARAM_GRID.keys())
    combos = list(itertools.product(*[TUNE_PARAM_GRID[k] for k in keys]))
    n_combos = len(combos)

    print(f'\n{"=" * 65}')
    print(f'  HYPERPARAMETER TUNING — GCN with Features')
    print(f'  Param grid: {TUNE_PARAM_GRID}')
    print(f'  Total combinations: {n_combos}')
    print(f'  Folds per combination: 8  (LOPO)')
    print(f'  Total training runs: {n_combos * 8}')
    print(f'  Epochs per run (tune): {args.tune_epochs}  '
          f'(final model uses {args.epochs})')
    print(f'  Selection criterion: mean inner_val_auc')
    print(f'  WARNING: outer test AUC will NOT be computed until')
    print(f'           the best params are confirmed.')
    print(f'{"=" * 65}')
    ...
    for combo_idx, combo in enumerate(combos):
        ...
        print(f'\n  [{combo_idx + 1:3d}/{n_combos}] Testing: {params}', end='  ', flush=True)

        # Run LOPO with tuning epochs and no plot saving
        fold_metrics, summary = run_gcn_lopo(
            model_tag          = 'tune_gcn_feat',
            in_dim             = 16,
            node_feats_or_none = node_feats,
            adj_dtf_all        = adj_dtf_all,
            y                  = y,
            patient_ids        = patient_ids,
            args               = args,
            device             = device,
            output_dir         = output_dir,
            save_plots         = False,      # suppress per-fold plots during tuning
        ...
        mean_iv = summary.get('inner_val_auc', {}).get('mean', float('nan'))
        std_iv  = summary.get('inner_val_auc', {}).get('std',  float('nan'))

        record = {
            'params':              params,
            'mean_inner_val_auc':  float(mean_iv),
            'std_inner_val_auc':   float(std_iv),
            'mean_test_auc':       float(summary.get('auc', {}).get('mean', float('nan'))),
            # NOTE: mean_test_auc is recorded here for logging/analysis purposes only.
            # It is NOT used for selection. Selection is solely on mean_inner_val_auc.
        }
        tuning_records.append(record)
        print(f'inner_val_auc = {mean_iv:.4f} ± {std_iv:.4f}')

    if not tuning_records:
        raise RuntimeError('No valid tuning combinations found.')

    # ── Select best params by mean inner_val_auc ──────────────
    best = max(tuning_records, key=lambda r: r['mean_inner_val_auc'])

    print(f'\n{"=" * 65}')
    print(f'  TUNING COMPLETE')
    print(f'  Best params (by inner_val_auc): {best["params"]}')
    print(f'  Best mean inner_val_auc: {best["mean_inner_val_auc"]:.4f} ± '
          f'{best["std_inner_val_auc"]:.4f}')
    print(f'  (Outer test AUC will be computed next with these params)')
    print(f'{"=" * 65}')

    # ── Save tuning results JSON ──────────────────────────────
    tuning_output = {
        'selection_criterion': 'mean_inner_val_auc (outer test AUC NOT used for selection)',
        'param_grid':          TUNE_PARAM_GRID,
        'n_folds':             8,
        'tune_epochs':         args.tune_epochs,
        'best_params':         best['params'],
        'best_inner_val_auc':  best['mean_inner_val_auc'],
        'all_combinations':    sorted(tuning_records,
                                      key=lambda r: r['mean_inner_val_auc'],
                                      reverse=True),
    }
    with open(output_dir / 'hyperparameter_tuning_results.json', 'w') as f:
        json.dump(tuning_output, f, indent=2, default=str)
    print(f'  Saved: hyperparameter_tuning_results.json')

    # ── Produce tuning visualisation plots ────────────────────
    plot_tuning_results(tuning_records, output_dir)

    return best['params'], tuning_records
...
def main():
    parser = argparse.ArgumentParser(
        description='Step 5 — Supervised GCN (Nested LOPO, data-driven threshold, '
                    'rotating inner val, optional hyperparameter tuning)'
    )
    parser.add_argument('--featfile',       required=True)
    parser.add_argument('--outputdir',      default='results/gnn_supervised')
    parser.add_argument('--epochs',         type=int,   default=150,
                        help='Training epochs for final model evaluation (default 150)')
    ...
    parser.add_argument('--tune',           action='store_true',
                        help='Run hyperparameter grid search on GCN with Features. '
                             'Selection is based on mean inner_val_auc — '
                             'outer test AUC is never used for selection.')
    parser.add_argument('--tune_epochs',    type=int, default=100,
                        help='Epochs to use during tuning grid search (default 100). '
                             'Fewer than --epochs to keep runtime manageable. '
                             'Final model always uses --epochs.')
    ...
    if args.tune:
        print('          + HYPERPARAMETER TUNING (GCN with Features)')
    print('=' * 65)
    ...
    if args.tune:
        print(f'Tune epochs   : {args.tune_epochs}  (used during grid search only)')
        print(f'Param grid    : {TUNE_PARAM_GRID}')
        print(f'N combos      : {len(list(itertools.product(*TUNE_PARAM_GRID.values())))}')
        print()
        print('TUNING INTEGRITY NOTE:')
        print('  Selection criterion = mean inner_val_auc across 8 outer folds.')
        print('  Outer test AUC is computed ONCE, AFTER param selection.')
        print('  This preserves the integrity of the final LOPO evaluation.')
    print()
    ...
    # ── Determine final hyperparams for GCN with Features ──────
    # Default: use args values directly (no tuning).
    # If --tune: run grid search first, then use selected params.
    feat_hidden        = args.hidden
    feat_dropout       = args.dropout
    feat_lr            = args.lr
    feat_threshold_pct = args.threshold_pct

    if args.tune:
        print('\n' + '=' * 65)
        print('PHASE 1: Hyperparameter Grid Search (inner_val_auc criterion)')
        print('=' * 65)
        best_params, tuning_records = run_hyperparameter_tuning(
        ...
        # Update final params from tuning result
        feat_hidden        = best_params['hidden']
        feat_dropout       = best_params['dropout']
        feat_lr            = best_params['lr']
        feat_threshold_pct = best_params['threshold_pct']

        print('\n' + '=' * 65)
        print('PHASE 2: Final Evaluation with Best Params (full epochs)')
        print(f'  hidden={feat_hidden}  dropout={feat_dropout}  '
              f'lr={feat_lr}  threshold_pct={feat_threshold_pct}')
        print(f'  Using {args.epochs} epochs (full training).')
        print('=' * 65)
    ...
    feat_metrics, feat_stats = run_gcn_lopo(
        ...
        lr                 = feat_lr,
        threshold_pct      = feat_threshold_pct,
        epochs             = args.epochs,
    )

    # ── Model B: Adjacency-only GCN (ablation) ──────────────────
    # NOTE: Adjacency-only model always uses original args (not tuned).
    # Tuning is applied to the GCN with Features only, which is the
    # primary model. The adj-only variant is an ablation — it gets the
    # same fixed hyperparameters used in the original run for fair comparison.
    adj_metrics, adj_stats = run_gcn_lopo(
        model_tag          = 'gcn_adj_only',
        in_dim             = 19,
        node_feats_or_none = None,
        adj_dtf_all        = adj_dtf,
        y                  = y,
        patient_ids        = patient_ids,
        args               = args,
    ...
    if args.tune:
        tuning_note = {
            'tuned':             True,
            'selection_criterion': 'mean_inner_val_auc (NOT outer test AUC)',
            'best_params':       {
                'hidden': feat_hidden, 'dropout': feat_dropout,
                'lr': feat_lr, 'threshold_pct': feat_threshold_pct,
            },
            'tune_epochs':       args.tune_epochs,
    ...
    for tag, metrics, stats, fname, used_hidden, used_dropout, used_lr, used_thr in [
        ('GCN_Supervised_WithFeatures', feat_metrics, feat_stats,
         'results_gcn_with_features.json',
         feat_hidden, feat_dropout, feat_lr, feat_threshold_pct),
        ('GCN_Supervised_AdjOnly',      adj_metrics,  adj_stats,
         'results_gcn_adj_only.json',
         args.hidden, args.dropout, args.lr, args.threshold_pct),
    ]:
        hp = {
            ...
            'lr':             used_lr,
            'hidden':         used_hidden,
            'dropout':        used_dropout,
            'threshold_pct':  used_thr,
        ...
        res = {
            'model':            tag,
            'hyperparameters':  hp,
            'threshold_note':   f'Data-driven p{used_thr:.0f} percentile per fold '
                                f'from inner-train adjacency',
            'inner_val_note':   'Rotating inner val patient — cycles with fold index '
                                'to avoid fixed validator bias',
            'fold_metrics':     metrics,
            'summary_stats':    stats,
        }
        if tuning_note and 'WithFeatures' in tag:
            res['hyperparameter_tuning'] = tuning_note
    ...
    with open(output_dir / 'results_gcn.json', 'w') as f:
        json.dump({
            ...
            'hyperparameters': {
                'featfile': args.featfile, 'outputdir': args.outputdir,
                'epochs': args.epochs, 'lr': feat_lr, 'hidden': feat_hidden,
                'dropout': feat_dropout, 'threshold_pct': feat_threshold_pct,
                'patience': args.patience, 'baseline_json': args.baseline_json,
            },
            'fold_metrics':    feat_metrics,
            'summary_stats':   feat_stats,
        }, f, indent=2, default=str)
    print(f'  Saved: results_gcn.json  (alias for step6)')
    ...
    print('=' * 65)
    if args.tune:
        print(f'  [Tuned] GCN with Features params: hidden={feat_hidden} '
              f'dropout={feat_dropout} lr={feat_lr} threshold_pct={feat_threshold_pct}')
        print(f'  [Fixed] Adj-only GCN params: default (no tuning applied to ablation)')
