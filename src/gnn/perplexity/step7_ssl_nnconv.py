...
=========================================================
Same as step7_ssl_graphsage.py but uses NNConv encoder
that incorporates edge attributes [DTF, PDC] in message passing.

Input : graphs_v2/all_graphs_dtf_topk6.pt  (from step5 v2)
Output: results_ssl_nnconv/
...
def augment_graph(data, mask_ratio=0.1, drop_ratio=0.1):
    ...
    edge_attr is kept for kept edges (NNConv needs it).
    """
    # Feature masking
    ...
    # Edge dropping — keep same edges in edge_attr
    ...
    return Data(
        ...
        edge_attr  = data.edge_attr[perm],     # ← keep matching edge_attr
    )
...
def run_lopo_finetune(graphs, pretrained_encoder, args, device, out_dir):
    ...
    for fold, test_pat in enumerate(patients):
        ...
        for epoch in range(args.freeze_epochs + 1,
...
def main(args):
    ...
    print(f"  Edge dim : {graphs[0].edge_attr.shape[1]}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Step 7 v2 — SSL NNConv")
