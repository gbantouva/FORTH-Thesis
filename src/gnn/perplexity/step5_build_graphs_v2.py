...
===============================================================
Changes from v1:
  - NNConv-compatible edge attributes: [dtf, pdc] (2-dim)
  - Option --mode dtf | pdc | both controls which measure
    is used for edge SELECTION (top-k). Both are always
    stored in edge_attr regardless.
  - edge_attr shape: (n_edges, 2) = [dtf_integrated, pdc_integrated]

Node features (24 per node) — unchanged from v1:
  DTF out/in strength x6 bands + PDC out/in strength x6 bands

Edge construction:
  top-k=6 outgoing edges per node, ranked by --mode measure
  edge_attr = [dtf_integrated, pdc_integrated] always stored
...
def node_features(data_npz, epoch_idx):
    ...
    Features: DTF out/in x6 bands, PDC out/in x6 bands.
    """
    feats = []
    for measure in ['dtf', 'pdc']:
        ...
        for band in BANDS:
            ...
            out_bands.append(mat.sum(axis=1))               # (19,)
            in_bands.append(mat.sum(axis=0))                # (19,)
...
def build_edges_topk(dtf_mat, pdc_mat, top_k=6, mode='dtf'):
    """
    For each node, keep top_k strongest outgoing edges.
    Ranking is based on --mode measure.
    edge_attr = [dtf_weight, pdc_weight] always stored.

    Returns
    -------
    edge_index : (2, K*top_k)   torch.long
    edge_attr  : (K*top_k, 2)   torch.float  [dtf, pdc]
    """
    ...
    for i in range(K):
        # rank outgoing edges from node i by chosen measure
...
def process_subject(npz_path, patient_id, top_k=6,
                    mode='dtf', ratio=2):
...
def build_dataset(conn_dir, out_dir, top_k=6, mode='dtf', ratio=2):
    ...
    print(f"Edge mode      : {mode.upper()} (top-{top_k} per node)")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Step 5 v2 — Graph construction with edge attributes")
    parser.add_argument('--conndir', required=True)
    parser.add_argument('--outdir',  required=True)
    parser.add_argument('--topk',    type=int,   default=6,
                        help='Top-k outgoing edges per node (default: 6)')
