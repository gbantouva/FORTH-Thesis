...
Pre-ictal sampling strategy (clinically motivated):
    ...
    Limit pre-ictal to PRE_ICTAL_RATIO × n_ictal epochs.

    Rationale:
        - Epochs at the start of the recording are maximally far from the
          seizure — clearly interictal, not transitional.
        - This makes the binary task cleaner:
          "clearly interictal EEG graph  vs  ictal EEG graph"
        - Reduces imbalance from ~12:1 to ~2:1 in a principled way.
        - Avoids arbitrarily capping or randomly discarding pre-ictal epochs.
...
Usage:
    python step5_build_dataset.py \
        ...
        --output_dir       F:/FORTH_Final_Thesis/FORTH-Thesis/gnn_dataset_2to1
"""
...
# For each subject: keep all ictal, keep this many × n_ictal pre-ictal epochs
# taken from the START of the recording (most negative time_from_onset).
PRE_ICTAL_RATIO = 2
...
def build_edge_index_and_weights(dtf_matrix: np.ndarray):
    """
    dtf_matrix : (19, 19)  diagonal=0, convention matrix[i,j] = j→i
...
def load_subject_graphs(subj_id, connectivity_dir, features_dir, ratio=2):
    ...
    # ── time_from_onset ───────────────────────────────────────────────────────
    # Used to pick the pre-ictal epochs farthest from the seizure.
    if 'time_from_onset' in graphs:
        time_from_onset = graphs['time_from_onset']   # (n_valid,)
    else:
        # fallback: reconstruct from labels (first ictal epoch = t=0)
        first_ictal = np.where(labels_all == 1)[0]
        if len(first_ictal) == 0:
            time_from_onset = np.arange(n_valid) * 4.0
        else:
            time_from_onset = (np.arange(n_valid) - first_ictal[0]) * 4.0
        print(f'  [{subject_name}] time_from_onset not in npz — reconstructed')
    ...
    # ── separate ictal and pre-ictal (masked) ─────────────────────────────────
    ictal_indices    = [i for i in range(n_valid)
                        if mask[i] and labels_all[i] == 1]
    preictal_indices = [i for i in range(n_valid)
                        if mask[i] and labels_all[i] == 0]

    n_ictal   = len(ictal_indices)
    n_pre_max = ratio * n_ictal

    # Sort pre-ictal by time_from_onset ascending
    # → most negative values first = farthest before seizure
    preictal_indices_sorted = sorted(
        preictal_indices,
        key=lambda i: time_from_onset[i]
    )

    # Take the first n_pre_max (from start of recording)
    selected_preictal = preictal_indices_sorted[:n_pre_max]
    ...
    for ep in selected_indices:
        edge_index, edge_attr = build_edge_index_and_weights(pdc_all[ep])
        graph = Data(
...
def main():
    ...
    parser.add_argument('--ratio', type=int,  default=2,
                        help='Pre-ictal epochs kept per ictal epoch (default=2)')
    args = parser.parse_args()

    #PRE_ICTAL_RATIO = args.ratio
    ...
    print('=' * 70)
    print(f'Pre-ictal sampling: {args.ratio}x ictal, from START of recording')
    ...
    for subj_id in all_subjects:
        ...
        split_graphs[split].extend(
            load_subject_graphs(subj_id, connectivity_dir, features_dir, ratio=args.ratio)
        )
    ...
    info = {
        ...
        'preictal_sampling'  : {
            ...
            'rationale': (
                ...
                'from seizure onset. Kept ratio × n_ictal epochs per subject.'
            ),
        },
