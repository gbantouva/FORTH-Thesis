...
Saves to output_dir:
    ...
    gcn_training_curves.png — loss + AUROC + sensitivity + specificity over epochs
    gcn_confusion_matrix.png — confusion matrix heatmap on test set

Usage:
    python step6_train_gcn.py \
        --dataset_dir F:/FORTH_Final_Thesis/FORTH-Thesis/gnn/gnn_dataset \
        --output_dir  F:/FORTH_Final_Thesis/FORTH-Thesis/gnn/results/gcn_v2 \
        --lr 0.0002 --dropout 0.5 --patience 50 --epochs 300
"""
...
def save_training_curves(history: list, best_epoch: int, out_path: Path):
    """4-panel figure: train loss, val AUROC, val sensitivity, val specificity."""
...
def save_confusion_matrix(metrics: dict, out_path: Path, model_name: str = 'GCN'):
...
def train(args):
    ...
    print(f'  Saved: gcn_results.json')

    # 2. training history
    np.save(output_dir / 'gcn_history.npy', np.array(history))
    print(f'  Saved: gcn_history.npy')
