...
Subject -> Patient mapping (TUC Focal Seizures dataset):
  subj 1        -> patient 1
  subj 2        -> patient 13
  subj 3-10     -> patient 14
  subj 11       -> patient 15
  subj 12-25    -> patient 24
  subj 26-32    -> patient 27
  subj 33       -> patient 29
  subj 34       -> patient 35
...
def matrix_features(mat):
    ...
    out_str  = mat.sum(axis=1)                                    # (19,)
    in_str   = mat.sum(axis=0)                                    # (19,)
    #stats    = np.array([mat.mean(), mat.std(), mat.max()])       # (3,)
    # With this:
    mask  = ~np.eye(K, dtype=bool)
    stats = np.array([mat[mask].mean(), mat[mask].std(), mat[mask].max()])
...
def select_epochs(labels_orig, ratio=2):
    ...
    - Pre-ictal (y=0): first (ratio * n_ictal) non-ictal epochs
                       sorted by index = earliest = furthest from seizure

    Returns
    -------
    keep_idx : indices into original epoch array
    y_binary : binary labels (0=pre-ictal, 1=ictal)
    """
    ...
    preictal_idx = non_ictal_idx[:n_preictal]
...
def extract_subject(npz_path, patient_id, ratio=2):
    ...
    for i in keep_idx:
        feat = []
        for band in BANDS:
            dtf = data[f'dtf_{band}'][i]
            pdc = data[f'pdc_{band}'][i]
    ...
    print(f"  subject_{npz_path.stem.split('_')[1]}  "
          f"(patient {patient_id:2d})  "
          f"total={len(labels_orig):4d} | kept={len(y_binary):4d} | "
          f"ictal={y_binary.sum():3d}  pre-ictal={(y_binary==0).sum():3d}")
...
def build_dataset(conn_dir, out_dir, ratio=2):
    ...
    for npz in npz_files:
        # Parse subject number: "subject_03_graphs.npz" -> 3
        try:
            subject_num = int(npz.stem.split('_')[1])
        except (IndexError, ValueError):
            print(f"  [WARN] Cannot parse subject number from {npz.name} — skipping")
            continue

        patient_id = SUBJECT_TO_PATIENT.get(subject_num)
        if patient_id is None:
            print(f"  [WARN] subject_{subject_num:02d} — no patient mapping, skipping")
            continue

        X, y, g = extract_subject(npz, patient_id=patient_id, ratio=ratio)
        if X is None:
            continue
