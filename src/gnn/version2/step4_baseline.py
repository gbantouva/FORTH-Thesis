...
def plot_roc_curve(all_y_true, all_y_proba, fold_aucs, model_name, out_dir):
    ...
    # note about pooled vs mean
    ax.text(0.55, 0.08,
            '* Pooled ROC concatenates all\n  fold predictions — patients\n'
            '  with more epochs dominate.\n  Mean fold AUC is more reliable.',
            fontsize=7.5, color='gray', transform=ax.transAxes,
...
def run_lopo(X, y, groups, clf_name, clf, out_dir):
    ...
    for fold, (train_idx, test_idx) in enumerate(logo.split(X, y, groups)):
        ...
        print(f"  Fold {fold+1:2d} | patient={test_patient:2d} "
              f"| n={len(y_te):3d} "
              f"(ict={int(y_te.sum()):2d} pre={int((y_te==0).sum()):2d}) "
...
def main(features_path, out_dir):
    ...
    for name, clf in models.items():
        summary_row, fold_records, cm_total = run_lopo(
            X, y, groups, name, clf, out_dir)
        all_summaries.append(summary_row)
        cm_totals[name] = cm_total
    ...
    print(f"{'='*65}")
    header = (f"  {'Model':15s}  {'Sens':>12}  {'Spec':>12}  "
              f"{'Prec':>12}  {'F1':>12}  {'BalAcc':>12}  {'AUC':>12}")
    print(header)
    print(f"  {'─'*78}")
    ...
    results_json = {
        'experiment': {
            'script'     : 'step4_baseline_ml.py',
            'timestamp'  : datetime.now().isoformat(timespec='seconds'),
            'features'   : str(features_path),
            'n_epochs'   : int(len(y)),
            'n_ictal'    : int((y == 1).sum()),
            'n_preictal' : int((y == 0).sum()),
            'patients'   : sorted(np.unique(groups).tolist()),
            'cv_strategy': 'LeaveOnePatientOut (LOPO)',
            'note_roc'   : (
                'Pooled ROC concatenates predictions across all folds. '
                'Patients with more test epochs dominate. '
                'Mean fold AUC is the primary reported metric.'
            ),
        },
        'hyperparameters': {
            name: clf.get_params()
            for name, clf in models.items()
        } | {'scaler': 'StandardScaler — fitted on train fold only, no leakage'},
        'results': {
            row['model']: {
                k: {
                    'mean': round(row[f'{k}_mean'], 4),
                    'std' : round(row[f'{k}_std'],  4),
                }
                for k in METRICS_KEYS
            }
            for row in all_summaries
        ...
        'confusion_matrices': {
            name: {
                'layout': '[[TN, FP], [FN, TP]]',
                'matrix': cm_totals[name].tolist(),
                'TN': int(cm_totals[name][0, 0]),
                'FP': int(cm_totals[name][0, 1]),
                'FN': int(cm_totals[name][1, 0]),
                'TP': int(cm_totals[name][1, 1]),
            }
            for name in cm_totals
        },
        'fold_details': all_fold_rows,
    }
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Step 4 — Baseline ML (LOPO)")
    parser.add_argument('--features', required=True,
                        help='Path to features/all_features.npz  (step3 output)')
    parser.add_argument('--outdir', default='results',
                        help='Output folder for all plots, CSVs and JSON')
