...
class GATv2Encoder(nn.Module):
    ...
    Layer 1+2: heads attention heads, concat=True
               output dim = hidden_channels  (heads * head_dim)
    Layer 3:   1 attention head, concat=False
               output dim = hidden_channels

    Edge attributes [dtf, pdc] are passed to GATv2Conv so attention
    scores are conditioned on both node features AND connectivity
    strength — key advantage over GraphSAGE for directed EEG graphs.
    """
...
def augment_graph(data, mask_ratio=0.1, drop_ratio=0.1):
    ...
    return Data(
        ...
        edge_attr  = data.edge_attr[perm],   # keep matching attr rows
    )
...
def nt_xent_loss(z1, z2, temperature=0.1):
    """
    NT-Xent contrastive loss for batch of N graphs.
    Positive pair : (z1[i], z2[i]) — two augmented views of graph i
...
def run_lopo_finetune(graphs, pretrained_encoder, args, device, out_dir):
    ...
    Stage A (epochs 1 → freeze_epochs):
      ...
      Stabilises head without disturbing pretrained weights.

    Stage B (epochs freeze_epochs+1 → finetune_epochs):
      Encoder UNFROZEN — both train together,
      encoder uses lr * 0.1 to preserve pretrained representations.
    """
...
def main(args):
    ...
    print(f"  Edge dim : {edge_dim}  ← used in GATv2 attention")
    ...
    results_json = {
        'experiment': {
            ...
            'edge_attr_used'  : True,
            'note'            : 'GATv2Conv uses edge_attr [dtf,pdc] in attention.',
        },
        'hyperparameters': {
            ...
            'activation'         : 'ELU',
            'normalization'      : 'BatchNorm1d after each GATv2Conv',
        },
        'results': {
            k: {
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Step 7 v3 — SSL GATv2 (GraphCL + Two-Stage Fine-tune)")
