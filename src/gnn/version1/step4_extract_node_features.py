...
def process_subject(
    ...
    meta = {
        "script"          : "step4_extract_node_features.py",
        "timestamp"       : datetime.now().isoformat(timespec='seconds'),
        "subject"         : subject_name,
        "n_valid_epochs"  : int(n_valid),
        "output_shape"    : list(node_features.shape),
        "features": {
            "0": "delta band power (log1p + zscore)",
            "1": "theta band power (log1p + zscore)",
            "2": "alpha band power (log1p + zscore)",
            "3": "beta  band power (log1p + zscore)",
            "4": "gamma band power (log1p + zscore)",
            "5": "DTF outflow integrated",
            "6": "DTF inflow  integrated",
            "7": "PDC outflow integrated",
            "8": "PDC inflow  integrated",
        },
        "normalization"   : "log1p on band powers, then z-score per subject",
        "fs_hz"           : FS,
        "nfft"            : NFFT,
        "bands_hz"        : BANDS,
    }
    meta_path = output_dir / f'{subject_name}_node_features_meta.json'
