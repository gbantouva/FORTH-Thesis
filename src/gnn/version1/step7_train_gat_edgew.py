...
============================================
Key fix over v1: passes DTF/PDC edge weights into the attention computation
...
Architecture:
    GATConv(9 → hidden, heads=4, edge_dim=1, concat=True)  → hidden*4
    BatchNorm → ELU → Dropout
    GATConv(hidden*4 → hidden, heads=1, edge_dim=1, concat=False) → hidden
    BatchNorm → ELU → Dropout
    GlobalMeanPool
...
Usage:
    python step7_train_gat_edgeaware.py \
        --dataset_dir F:/FORTH_Final_Thesis/FORTH-Thesis/gnn/dataset_dtf \
        --output_dir  F:/FORTH_Final_Thesis/FORTH-Thesis/gnn/results/gat_edgeaware
"""
...
class GATClassifier(nn.Module):
    """
    2-layer GAT with edge-aware attention (edge_dim=1).
    DTF/PDC edge weights are fed into the attention score computation,
    so high-connectivity edges receive higher attention by default,
    while the model can still learn to override this from node features.
    """
    ...
    def forward(self, data: Data, return_attention=False):
        ...
        edge_attr  = data.edge_attr   # (n_edges, 1)
        batch      = data.batch
...
def evaluate(model, loader, device):
    ...
    return {
        ...
        'tp': int(tp), 'tn': int(tn),
        'fp': int(fp), 'fn': int(fn),
        'probs'    : probs,       # for plots — excluded from JSON
        'labels_np': labels_np,   # for plots — excluded from JSON
    }
...
def save_training_curves(history, best_epoch, out_path):
    ...
    fig.suptitle('GAT (edge-aware) Training Curves', fontsize=14, fontweight='bold')
...
def save_overfitting_curves(history, best_epoch, out_path):
    ...
    fig.suptitle('GAT (edge-aware) — Overfitting Diagnostic',
...
def save_attention_plot(pre_attn, ict_attn, out_path):
    ...
    pairs = [(diff[i,j], CHANNEL_NAMES[j], CHANNEL_NAMES[i])
             for i in range(19) for j in range(19) if i != j]
...
def train(args):
    ...
    results = {
        'experiment': {
            'script'     : 'step7_train_gat_edgeaware.py',
            'timestamp'  : datetime.now().isoformat(timespec='seconds'),
            'model'      : 'GAT_edge_aware',
            'description': (
                '2-layer GAT with edge_dim=1. DTF/PDC edge weights are passed '
                'into the attention score computation, so connectivity strength '
                'directly modulates which channel messages are aggregated.'
            ),
        },
        'hyperparameters': {
            'hidden'      : args.hidden,
    ...
    save_roc_curve(test_m['labels_np'], test_m['probs'], test_m_json,
                   output_dir / 'gat_roc_curve.png', model_name='GAT (edge-aware)')
    save_confusion_matrix(test_m_json,
                          output_dir / 'gat_confusion_matrix.png',
                          model_name='GAT (edge-aware)')
