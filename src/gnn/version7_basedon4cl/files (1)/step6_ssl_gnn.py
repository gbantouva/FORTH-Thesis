...
SSL pre-training phase:
  A) Contrastive learning as an implicit regulariser:
     The NT-Xent loss does not use labels. By learning representations
     that are invariant to graph augmentations, the encoder is forced
     to capture structural properties of EEG connectivity graphs rather
     than patient-specific idiosyncrasies. This is the primary way SSL
     reduces overfitting compared to direct supervised training.

  B) Three EEG-appropriate augmentations:
     - Edge dropout (p=0.20): randomly zeros edges, simulating missing
       connectivity; forces encoder not to rely on any single edge.
     - Node feature noise (σ=0.10): Gaussian noise on node features,
       simulating electrode noise; prevents learning exact feature values.
     - Band feature mask: zeros one random frequency band for all nodes,
       preventing the encoder from anchoring to a single EEG band.
     Together these create a diverse augmentation distribution that
     acts like strong data augmentation regularisation.

  C) CosineAnnealingLR during SSL:
     Smoothly decays learning rate to lr×0.1, preventing overfitting
     to augmented views in later epochs.

  D) Gradient clipping (max_norm=1.0):
     Applied during SSL pre-training. With batch sizes of 32 graph-level
     embeddings, gradient variance is high — clipping prevents updates
     that overfit a single batch.

Fine-tuning phase:
  E) Two-phase fine-tuning (linear probe then full fine-tune):
     Phase A (frozen encoder, 30 epochs): only the classification head
     is trained. This prevents the SSL encoder from being distorted
     by the small labelled training set before the head is calibrated.
     Phase B (full fine-tune, lr_ft=0.0005): smaller learning rate
     makes fine-tuning updates conservative, preserving SSL-learned
     representations.

  F) Dropout (p=0.4) and L2 weight decay (1e-4):
     Same as step 5 — applied in both encoder and classifier head.

  G) Early stopping on INNER val AUC (patience=20):
     Fine-tuning halts when inner-val AUC stops improving.
     The test patient is never consulted during training.

  H) BCEWithLogitsLoss with pos_weight:
     pos_weight = n_neg/n_pos (computed from inner-train only).
     Prevents the dominant pre-ictal class from dominating the loss.

  I) Node feature scaling:
     StandardScaler fit on inner-train graphs only.
     Applied to inner-val and test graphs using train statistics.

  J) Small model (identical to step 5, ~4k parameters):
     Keeps capacity low relative to dataset size.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
OVERFITTING DETECTION — PLOTS:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ssl_loss_fold_{patient}.png
      NT-Xent contrastive loss over SSL pre-training epochs.
      Should decrease monotonically. A plateau indicates the encoder
      has reached its capacity limit (expected — model is small by design).
      A sudden rise would indicate instability (not expected).

  ssl_auc_probe_{patient}.png         ← NEW
      After SSL pre-training, a linear probe (frozen encoder) is
      evaluated on inner-val graphs epoch by epoch.
      This shows the *quality* of SSL representations:
      - High linear probe AUC early → encoder learned discriminative
        features without labels (SSL succeeded).
      - Low linear probe AUC → SSL did not extract useful representations
        → full fine-tuning must do more work.

  ft_training_curves_{patient}.png    ← NEW (replaces ft_loss_*)
      Two-panel: fine-tune loss + fine-tune AUC (train vs inner-val).
      Same interpretation as step 5 training_curves plots.

  train_vs_test_gap_ssl.png           ← NEW
      Train AUC / inner-val AUC / outer-test AUC per fold.

  overfitting_summary_ssl.png         ← NEW
      (train AUC − test AUC) per fold.

Outputs:
  results_ssl.json
  ssl_loss_fold_{patient}.png
  ssl_auc_probe_{patient}.png
...
class GCNEncoder(nn.Module):
    ...
    def forward(self, x, a):
        ...
        h = self.drop(h)
        h = self.gcn2(h, a)
        h = self.drop(h)           # dropout after both GCN layers
        return h.mean(dim=0)
...
def ssl_pretrain(encoder, proj_head, train_graphs, ssl_epochs, lr,
    ...
    """
    Pre-train on INNER TRAIN graphs only.
    Returns per-epoch NT-Xent loss.
    """
...
def finetune(encoder, clf_head,
             g_train, y_train, g_val, y_val,   # val = INNER, not test
             ft_epochs, lr, pos_weight,
             device, patience=20, freeze_encoder=False):
    """
    Early stopping monitored on INNER VAL — test patient is never seen.
    Returns train_losses, val_losses, train_aucs, val_aucs, best_val_auc.
    """
    if freeze_encoder:
...
# ─────────────────────────────────────────────────────────────
# Linear probe — used after SSL to measure representation quality
# ─────────────────────────────────────────────────────────────

@torch.no_grad()
def linear_probe_auc(encoder, graphs, labels, device):
    """
    Compute AUC using encoder embeddings + a simple dot-product.
    Used to assess SSL representation quality without fine-tuning.
    """
    encoder.eval()
    embeddings = np.stack([
        encoder(x.to(device), a.to(device)).cpu().numpy()
        for x, a in graphs
    ])
    labels = np.asarray(labels)
    # Simple logistic-like scoring: project onto mean ictal − mean pre-ictal direction
    pos_mean = embeddings[labels == 1].mean(axis=0) if (labels == 1).any() else embeddings.mean(axis=0)
    neg_mean = embeddings[labels == 0].mean(axis=0) if (labels == 0).any() else embeddings.mean(axis=0)
...
def plot_ssl_loss_and_probe(ssl_losses, probe_auc, patient_id, output_dir):
    """
    Two-panel SSL overview:
      Left : NT-Xent loss over pre-training epochs.
      Right: Bar showing linear probe AUC on inner-val after SSL.

    Interpretation for thesis:
      - Falling NT-Xent loss: encoder is learning to distinguish augmented
        views from other graphs (good).
      - Probe AUC > 0.5 after SSL (before any fine-tuning): the encoder
        has learned representations that are already somewhat discriminative
        without seeing a single label — the key SSL benefit claim.
      - Probe AUC ≈ 0.5: SSL learned a useful generic structure, but
        labelled fine-tuning is needed to orient it towards the ictal class.
    """
    ...
    final_drop = ssl_losses[0] - ssl_losses[-1] if ssl_losses else 0
    ax.text(0.97, 0.97, f'Total drop: {final_drop:.3f}',
    ...
    plt.suptitle(f'SSL quality | Test patient: {patient_id}',
...
def plot_ft_training_curves(train_losses, val_losses, train_aucs, val_aucs,
    ...
    """
    Two-panel fine-tune diagnostic (loss + AUC) — same format as step 5.
    ...
    ax.set_title(f'Fine-tune loss | Test: {patient_id}', fontsize=10, fontweight='bold')
    ...
    ax.set_title(f'Fine-tune AUC | Test: {patient_id}', fontsize=10, fontweight='bold')
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)

    plt.suptitle(f'SSL-GCN Fine-tuning Overfitting Diagnostic | Test: {patient_id}',
                 fontsize=11, fontweight='bold', y=1.01)
    plt.tight_layout()
    plt.savefig(output_dir / f'ft_training_curves_{patient_id}.png',
...
def plot_train_vs_test_gap(fold_metrics, output_dir):
    ...
    ax.set_title('SSL-GCN — Train / Inner-val / Test AUC per fold\n'
                 '(Δ = train−test gap; near zero = no overfitting)',
...
def plot_overfitting_summary(fold_metrics, output_dir):
    ...
    ax.set_title('SSL-GCN — Overfitting Gap per Patient Fold\n'
                 '(Red bars > 0.10 indicate potential memorisation)',
                 fontsize=11, fontweight='bold')
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3, axis='y')
...
def main():
    parser = argparse.ArgumentParser(description='Step 6 — SSL GCN (leakage-free, nested LOPO, overfitting analysis)')
    ...
    print('=' * 65)
    print('STEP 6 — SSL GCN (leakage-free, nested LOPO, overfitting analysis)')
    print('=' * 65)
    ...
    print('  SSL: contrastive augmentation regularisation (edge_dropout, node_noise, band_mask)')
    print('  SSL: CosineAnnealingLR, gradient clipping(1.0), weight_decay=1e-4')
    print('  FT : two-phase (linear probe then full), dropout(0.4), early stopping(20)')
    print('  FT : early stopping on INNER val patient (test patient never seen)')
    print('  All: StandardScaler fit on inner-train only')
    print()
    print('Overfitting detection plots:')
    print('  ssl_overview_{patient}.png       — NT-Xent loss + linear probe AUC')
    print('  ft_training_curves_{patient}.png — FT loss + AUC (train vs inner-val)')
    print('  train_vs_test_gap_ssl.png        — train/val/test AUC bar chart')
    print('  overfitting_summary_ssl.png      — gap per fold')
    ...
    for fold_num, pat in enumerate(patients):
        ...
        print(f'  Train: {len(train_idx)} | Test: {len(test_idx)} | Inner val: {inner_val_pat}')
        ...
        print(f'\n  Phase 1 — SSL ({args.ssl_epochs} epochs, {len(g_it)} graphs, '
              f'no labels, no test data)')
        ...
        ssl_losses = ssl_pretrain(
            encoder, proj_head, g_it,   # inner train only
            ssl_epochs=args.ssl_epochs,
            lr=args.lr_ssl, device=device,
            batch_size=args.batch_size,
            temperature=args.temperature,
            verbose=True,
        )

        # Linear probe on inner-val (measure SSL representation quality)
        probe_auc = linear_probe_auc(encoder, g_iv, y_iv, device)
        print(f'  SSL final NT-Xent: {ssl_losses[-1]:.4f}  |  '
              f'Linear probe AUC (inner val): {probe_auc:.3f}')
        plot_ssl_loss_and_probe(ssl_losses, probe_auc, pat, output_dir)

        # Discard projection head — keep only encoder
        clf_head = ClassifierHead(in_dim=args.hidden, dropout=args.dropout).to(device)

        # ── Phase 2A: Linear probe fine-tune (frozen encoder) ────
        ...
        print(f'\n  Phase 2B — Full FT ({args.ft_epochs} epochs, early stop on inner val)')
    ...
    tr_aucs = [m['train_auc'] for m in fold_metrics if not np.isnan(m.get('train_auc', 0))]
    if tr_aucs:
    ...
    results = {
        ...
        'overfitting_prevention': [
            ...
            'early_stopping(patience=20) on inner_val_patient',
